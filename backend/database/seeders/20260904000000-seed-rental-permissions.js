...
const RENTAL_PERMISSIONS = [
    ['rentals.view', 'View rental agreements'],
    ['rentals.create', 'Create rental agreements'],
    ['rentals.return', 'Process rental returns'],
    ['rentals.cancel', 'Cancel rental agreements'],
];

export async function up(queryInterface) {
    for (const [name, description] of RENTAL_PERMISSIONS) {
    ...
    const [permissions] = await queryInterface.sequelize.query(
        `SELECT id, name FROM permissions WHERE name LIKE 'rentals.%'`
    );
...
export async function down(queryInterface) {
    await queryInterface.bulkDelete('permissions', {
        name: RENTAL_PERMISSIONS.map(([name]) => name),
    });
}
