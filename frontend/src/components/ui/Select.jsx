 ...
 *
 * This replaces the old native <select> wrapper: every select element in the app
 * must use this component (or its alias SearchableSelect), NOT a bare <select>.
 *
 * Renders a trigger button styled like a form control; clicking it opens a popover
 * with a search input and a filterable list. Supports an inline "+ Create <query>"
 * row when `creatable` and the query matches nothing.
 *
 * Props:
 * - value: selected option value (or undefined)
 * - onChange: (value) => void — receives the raw option value (NOT a DOM event)
 * - options: [{ value, label, description?, keywords? }]
 * - creatable / createLabel(query) / onCreate(query): inline create row
 * - label, placeholder, searchPlaceholder, emptyMessage, disabled, dataTestid, className
 */
const triggerCls =
