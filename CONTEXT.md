...
- **v1.1.0 release (2026-09-20): R-67** — Wi-Fi / LAN Network Access Modal & Dynamic IP QR (code integrated, tagged v1.1.0): added `GET /api/system/network` (returns active IPv4 interfaces, server port, primary URL; CORS allows private LAN origins 192.168.*, 10.*, 172.16-31.*, localhost) and frontend `NetworkAccessModal` accessible via a `Wifi` icon button in both desktop & mobile headers in `AppShell.jsx`. Renders dynamic QR code (`qrcode.react`), monospace URL box, 1-click Copy button with toast, network interface picker, and 3-step connection guide. Verified: all 48 vitest files (442 tests) pass, vite build clean, all 52 backend jest test suites (817 tests) pass. Tagged `v1.1.0` on `main` `6fb1219` + context `836855c`.

- **151 tickets done. Latest (2026-09-18, user sign-off): R-66** — R-52 interim: hide the Buying templates UI (FE only, reversible) — removed the `/trips/:tripUuid/templates` route + `TemplateForm` import in `App.jsx`, the *Pre-fill from a buying template* picker / *No buying templates saved* note / *Manage buying templates* button (+ their state/fetch/handler) in `StockForm.jsx`, and the 2 StockForm tests for that UI. Kept intact for restore: `TemplateForm.jsx`, `templatesApi.js`, `TEMPLATE_ROUTES`, the backend module, and `stock_templates` data. Verified vitest 434/434 + build clean; on context `4ba5b78` + main `634d87e`. R-52 stays blocked till ~mid-Oct fate decision.
- **Same day (2026-09-18): SemVer baseline took effect** — **v1.0.0** tagged on `main` (annotated `2c48ad6`) and **first GitHub Release published** at `https://github.com/rider4585/IMPOC/releases/tag/v1.0.0` (created from CLI). `gh` 2.101.0 installed at `/opt/homebrew/bin/gh`; use `GH_TOKEN="$(security find-internet-password -s github.com -w)" gh ...` (keychain token lacks `read:org`, so interactive `gh auth login` won't validate — repo-scoped ops work fine). Recipe in `docs/VERSIONING.md`. Docs-only changes → no bump.
- **Done (2026-09-17, user sign-off; laptop run still to do): R-60** durable backups — `pg_dump` **twice daily at 14:00 + 21:00** (local + encrypted Google Drive via rclone), **log-driven catch-up** (missed slot reruns at next logon, per-kind lock vs StartWhenAvailable), first backup during setup. Single source of truth `$script:BackupSlotTimes` in `lib/backup-common.ps1`; tasks registered by `lib/backup-setup.ps1` (old v1 names auto-removed). Already-setup laptop: run `setup-backup-local.cmd` then `setup-backup-cloud.cmd` as admin. Commit `9f2aa17` on main + context. Static review only — real test is the shop laptop. See `docs/WINDOWS_PRODUCTION_SETUP.md` §8.
- **Done (2026-09-17): R-65** enquiry close — **WhatsApp-only composer** (temp, pre-R-62): closing as available offers only WhatsApp (Email/SMS hidden FE-side), a composer window lets the counter write/edit the message, and Send opens `wa.me` with the typed text; also fixed the close **500** (`delivery_logs.entity_type` CHECK from `20260905000004` lacked `ENQUIRY`; new migration `20260917000001`, plus `tests/utils/test-setup.js` now mirrors the delivery-log CHECKs); **R-64** barcode PDF always fresh — removed the `request_keys` idempotency replay so `GET /api/barcodes/generate` never returns a cached JSON marker on cold start (shipped both branches); (2026-09-16): **R-63** Customer Enquiries (see below); (2026-09-14): **R-61** scanner zoom presets 1×/2×/3× (default 2×, remembered per device); **R-59** Units page — Scan button finds a unit by camera; **R-58** theme drawer — editable shop name + logo (server-side, public `GET /api/branding`, admin `branding.manage`) and a custom accent colour picker; **R-57** add-unit page — new colour/size can be created inline from the (searchable) dropdowns; **R-56** sheet configurator templates (`barcode_layout_templates`) + page sizes A3/A4/A5/Letter/Custom (mm/cm/inch, stored mm; no 10-row cap — fit check rules); **R-55** vendor bill photo — Take photo (in-app camera, native fallback) / gallery → review / retake, downscaled to 1600 px JPEG (`components/PhotoCapture.jsx` is the reusable image input; the only file input in the app, still base64 in `trip_vendors.receipt_image`); **R-54** Google review QR on the customer display after payment (held until POS *Close transaction*; link in Picklists → Review links, `review_links` table); **R-53** digital pet on the customer display (weighted random animations, per-row fps, tap-to-poke); **R-51** GST on purchases — CGST/SGST % on stocks, CGST/SGST ₹ on vendor bills (total paid is GST-inclusive; no reconciliation), live “cost per unit incl. GST” hint on the stock form (migration `20260913000004`); **R-48** barcode values `SHREE` + base-36 timestamp + base-36 counter (15 chars, e.g. `SHREE0D4N5H000V`, unique even if `barcode_seq` is reset; columns widened to 32); **R-49** Print labels stays usable after a download (toast + fresh request key) and the label's price box takes the space under the barcode; **R-50** barcode sheet configurator — single-row `barcode_layouts` table (mm), "Configure barcode sheet" section on Print labels with live SVG preview + server-rendered `GET /api/barcodes/preview`; the generator no longer reads `app_settings` barcode keys. Plus friendly wording for backend-down/5xx errors (`apiClient.friendlyServerMessage`). Before that: UPI-QR checkout + customer display (R-35), POS display polish (R-42), barcode scanner (R-41), mobile responsiveness (R-44), intake fix (R-43), colourful A5 receipt (R-45).
...
- **Parked by user (revisit ~2026-10-13):** **R-52** — hide / remove / keep the Buying templates module, after a month of production use. Interim (2026-09-18, user-verified): the templates UI is hidden FE-only (R-66, done) — functionality, backend and data fully intact for a later keep/restore decision.
...
| `tasks.json` (single file, incl. archived) | The kanban ticket log — who did what, per ticket, with detail. |
