...
const DELIVERY_PERMISSIONS = [
    ['delivery.view', 'View delivery logs'],
    ['delivery.create', 'Create delivery logs'],
];

export async function up(queryInterface) {
    for (const [name, description] of DELIVERY_PERMISSIONS) {
...
export async function down(queryInterface) {
    await queryInterface.bulkDelete('permissions', {
        name: DELIVERY_PERMISSIONS.map(([name]) => name),
    });
}