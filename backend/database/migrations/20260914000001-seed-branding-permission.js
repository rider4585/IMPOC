...
export async function up(queryInterface, Sequelize) {
    // R-58: find or create the BRANDING_MANAGE permission (Admin only)
    let permission = await queryInterface.sequelize.query(
        `SELECT id FROM permissions WHERE name = 'branding.manage' LIMIT 1`,
    ...
    if (permission.length === 0) {
        ...
        const result = await queryInterface.sequelize.query(
            `INSERT INTO permissions (uuid, name, description, created_at, updated_at)
             VALUES (gen_random_uuid(), 'branding.manage', 'Permission to edit the shop name and logo', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
             RETURNING id`,
            { type: queryInterface.sequelize.QueryTypes.SELECT }
        );
