...
- **Tests:** frontend `npx vitest run` (421 passing); backend `NODE_ENV=test NODE_OPTIONS=--experimental-vm-modules npx jest --forceExit` (753 passing).
...
- **In test:** **R-61** scanner zoom presets 1×/2×/3× (default 2×), remembered per device in localStorage, applied only when the camera can zoom.
