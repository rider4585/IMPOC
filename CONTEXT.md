...
- **In test (shipped v2, awaiting the laptop run):** **R-60** durable backups — `pg_dump` **twice daily at 14:00 + 21:00** (local + encrypted Google Drive via rclone), **log-driven catch-up** (missed slot reruns at next logon, per-kind lock vs StartWhenAvailable), first backup during setup. Single source of truth `$script:BackupSlotTimes` in `lib/backup-common.ps1`; tasks registered by `lib/backup-setup.ps1` (old v1 names auto-removed). Already-setup laptop: run `setup-backup-local.cmd` then `setup-backup-cloud.cmd` as admin. Commit `9f2aa17` on main + context. Static review only — no pwsh on this host, real test is the shop laptop. See `docs/WINDOWS_PRODUCTION_SETUP.md` §8.
