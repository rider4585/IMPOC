import { paiseToWordsINR } from './amountInWords.js';
import { RECEIPT_LOGO_SRC } from './receiptBrand.js';

/**
 * Colourful, branded A5 HTML receipt (R-45) — an elegant fashion-boutique
 * bill styled after `frontend/doc/receipt-ref.jpeg`: cream/ivory page, warm
 * brown ink, tan table header, dotted fill lines, faint lotus corner
 * watermarks. Deliberately does NOT use the app's dark theme tokens — this
 * is a printed/emailed document with its own fixed palette.
 *
 * `buildBrandedReceiptHtml(receipt)` returns a SELF-CONTAINED HTML string
 * (inline CSS, inline SVG only, system serif font stack) safe to render in
 * an iframe, print, or email (R-47) — no external images/fonts/CSS.
 *
 * `receipt` is the same payload GET /receipts/preview returns:
 * { store: {name, address, phone},
 *   transaction: {type, number, date, paymentMethod, totalPaise, ...},
